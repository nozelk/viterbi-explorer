...
function buildRow(states, initialValue) {
    return Object.fromEntries(states.map((state) => [state, initialValue]));
}

function runViterbiModel(states, obsSeq, startP, transP, emitP) {
    const T = obsSeq.length;
    const V = Array.from({ length: T }, () => buildRow(states, 0));
    const B = Array.from({ length: T }, () => buildRow(states, null));
    const steps = [];

    const firstObs = obsSeq[0];
    for (const state of states) {
        V[0][state] = startP[state] * emitP[state][firstObs];
        B[0][state] = null;
    }
    ...
    steps.push({
        ...
        V: { ...V[0] },
        B: { ...B[0] },
        explanation: `Inicializacija (t=0, opazovanje = '${firstObs}'): V[0][s] = π(s) · b_s(${firstObs}) za vsako stanje s.`,
        cells: states.map((state) => ({
            state,
            value: V[0][state],
            formula: `π(${state})·b_${state}(${firstObs}) = ${startP[state].toFixed(3)}·${emitP[state][firstObs].toFixed(3)} = ${V[0][state].toFixed(4)}`,
            from: null,
        })),
    });
    ...
    for (let t = 1; t < T; t += 1) {
        ...
        for (const state of states) {
            ...
            for (const prevState of states) {
                const value = V[t - 1][prevState] * transP[prevState][state] * emitP[state][obs];
                candidates.push({
                    from: prevState,
                    value,
                    formula: `V[${t - 1}][${prevState}]·a(${prevState}→${state})·b_${state}(${obs}) = ${V[t - 1][prevState].toFixed(4)}·${transP[prevState][state].toFixed(3)}·${emitP[state][obs].toFixed(3)} = ${value.toFixed(5)}`,
                });
            ...
            cells.push({
                ...
                from: bestPrev,
                candidates,
                formula: `max → prihaja iz '${bestPrev}', vrednost = ${bestVal.toFixed(5)}`,
            });
        }
        ...
        steps.push({
            ...
            V: { ...V[t] },
            B: { ...B[t] },
            explanation: `Korak t=${t}, opazovanje = '${obs}': za vsako stanje s izračunamo max_sp V[${t - 1}][sp]·a(sp→s)·b_s(${obs}) in shranimo najboljšega predhodnika.`,
            cells,
        });
    }

    const last = T - 1;
    const finalState = states.reduce((bestState, state) => (
        V[last][state] > V[last][bestState] ? state : bestState
    ), states[0]);
    const path = [finalState];

    for (let t = last; t > 0; t -= 1) {
        path.push(B[t][path[path.length - 1]]);
    }
    path.reverse();
    ...
    steps.push({
        ...
        V: null,
        B: null,
        explanation: `Backtracking: najboljša končna verjetnost je ${bestProb.toFixed(5)} v stanju '${finalState}'. Po backpointerjih sledimo nazaj → pot: ${path.join(" → ")}.`,
        path,
        best_prob: bestProb,
